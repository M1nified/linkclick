...
function shall_lock()
{
    if (preg_match('/\/wp-content\/uploads\//m', $_SERVER['REQUEST_URI']) === 1) {
        ...
        error_log("[".date('Y-m-d H:i:s')."][".__FUNCTION__."][".__LINE__."] ".print_r([].var_export($post_id, true), true)."\n", 3, __DIR__.'\..\..\debug.dev.log');
        error_log("[".date('Y-m-d H:i:s')."][".__FUNCTION__."][".__LINE__."] ONE: ".print_r([$is_access].var_export($is_access, true), true)."\n", 3, __DIR__.'\..\..\debug.dev.log');
        if ($is_access === true) {
            // error_log("[".date('Y-m-d H:i:s')."][".__FUNCTION__."][".__LINE__."] ".print_r(['GRANTED'], true)."\n", 3,  __DIR__.'\..\..\debug.dev.log');
            ...
            // error_log("[".date('Y-m-d H:i:s')."][".__FUNCTION__."][".__LINE__."] ".print_r(['DENIED'], true)."\n", 3,  __DIR__.'\..\..\debug.dev.log');
            $url = get_permission_denied_permalink($_SERVER['REQUEST_URI'], null, $is_access);
            // error_log("[".date('Y-m-d H:i:s')."][".__FUNCTION__."][".__LINE__."] ".print_r([$url], true)."\n", 3,  __DIR__.'\..\..\debug.dev.log');
            wp_redirect( $url, 302 );
            // auth_redirect();
            exit();
        }
...
{
    // error_log("[".date('Y-m-d H:i:s')."][".__FUNCTION__."][".__LINE__."] ".print_r(func_get_args(), true)."\n", 3,  __DIR__.'\..\..\debug.dev.log');
