...
function get_post_id_for_url($url)
{
    // echo $url;
    global $wpdb;
    $post = $wpdb->get_row("SELECT ID FROM {$wpdb->posts} WHERE guid LIKE '%{$url}%'");
    if ($post === null) {
        return false;
    } else {
        return $post->ID;
...
function is_access_url($url, $log_if_granted = false)
{
    ...
    error_log("[".date('Y-m-d H:i:s')."][".__FUNCTION__."][".__LINE__."] ".print_r([func_get_args(),$post_id], true)."\n", 3, __DIR__.'\..\..\debug.dev.log');
...
function is_access($post_id, $log_if_granted = false)
{
    ...
    error_log("[".date('Y-m-d H:i:s')."][".__FUNCTION__."][".__LINE__."] ".print_r([func_get_args(),$lock_id], true)."\n", 3, __DIR__.'\..\..\debug.dev.log');
    ...
    error_log("[".date('Y-m-d H:i:s')."][".__FUNCTION__."][".__LINE__."] ".print_r([func_get_args(),$is_access, $lock_id], true)."\n", 3, __DIR__.'\..\..\debug.dev.log');
...
{
    // error_log("[".date('Y-m-d H:i:s')."][".__FUNCTION__."][".__LINE__."] ".print_r([$url, $post, $is_access_result], true)."\n", 3,  __DIR__.'\..\..\debug.dev.log');
    $ret_url = $url;
